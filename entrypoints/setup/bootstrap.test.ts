...
describe("setup/bootstrap", () => {
  ...
  beforeEach(() => {
    ...
    delete (window as Record<string, unknown>).__OMH_SETUP_BOOT_STATE__;
  ...
  it("renders error UI when the timeout fires", async () => {
    ...
    expect(root.innerHTML).toContain("설정 화면을 시작하지 못했습니다");
    expect(root.querySelector("#setup-retry-btn")).not.toBeNull();
  ...
  it("clears timeout and sets appMounted on successful import", async () => {
    ...
    expect(window.__OMH_SETUP_BOOT_STATE__?.appMounted).toBe(true);
  });
  ...
  it("sets scriptLoaded on the boot state", async () => {
    ...
    expect(window.__OMH_SETUP_BOOT_STATE__?.scriptLoaded).toBe(true);
  });
