...
const PAGE_REQUEST_SOURCE = "openmyhealth-page";
const PAGE_RESPONSE_SOURCE = "openmyhealth-extension";
const PAGE_READ_REQUEST_TYPE = "openmyhealth:mcp:read-health-records";
const PAGE_READ_RESPONSE_TYPE = "openmyhealth:mcp:read-health-records:result";
const PAGE_READY_TYPE = "openmyhealth:mcp:ready";
...
function postBridgeMessage(port: MessagePort, payload: BridgeResponseEnvelope): void {
...
function isBridgeRequestMessage(data: unknown): data is BridgeRequestEnvelope {
...
export function setupPageMcpBridge(getProvider: () => AiProvider): () => void {
  const listener = (event: MessageEvent<unknown>) => {
    ...
    const parsed = ReadHealthRecordsRequestSchema.safeParse(event.data.payload);
    if (!parsed.success) {
